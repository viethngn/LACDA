...
class LACDA_Core:
	...
	def get_reach(self, Permalink):
		...
		unpaid_index = self.get_col_index("post_impressions_unique:lifetime", self.main_sheet)
		paid_index = self.get_col_index("post_impressions_paid_unique:lifetime", self.main_sheet)
	...
	def get_reaction_ratio(self, Permalink):
		...
		for index in range(self.main_sheet.nrows):
			if self.main_sheet.cell(index, link_index).value == Permalink:
				ratio['like'] = (self.get_like(Permalink))/reach
				ratio['share'] = (self.get_share(Permalink))/reach
				ratio['comment'] = (self.get_comment(Permalink))/reach
				return ratio
	...
	def get_promotion_ratio(self, Permalink):
		...
		for index in range(self.main_sheet.nrows):
			if self.main_sheet.cell(index, 1).value == Permalink:
				...
				promotion_ratio['reach'].append((self.main_sheet.cell(index, unpaid_index).value - self.main_sheet.cell(index, paid_index).value) / self.main_sheet.cell(index, unpaid_index).value)
